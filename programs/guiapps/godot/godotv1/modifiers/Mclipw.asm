...
gw0         lda inmerk		; get flag for type of inputted value
            tax			; save to .x
            eor #3
            tay
            txa			; was row?
            ...
            lda int		; no, value was zero?
            beq gw2		; yes, finish
            bne gw4		; else proceed

gw1         lda sc_clipzl,y	; was row+ver>max or was hor+col>max?
gw3         clc
            adc int
            sta int+1
            sec
            lda max-2,y
            sbc int+1
            bcc gw2		; yes, no change
            lda int
            sta clipundo,x
gw2         jmp ss53		; and show value
;
gw4         lda sc_clipzl,y	; care for right value
            pha
            tya
            eor #3
            tay
...
sevclclip   ldx sc_screenvek	; save current screenlist (ClipWorks)
            ...
            stx sc_br
            stx sc_ho
            jsr esm3		; compute address and invert
...
ml4         clc			; compute height
            ...
            stx data
            jsr zero1
ml5         lda sc_clipsp	; get indent, clip from left border?
            beq ml6		; yes
            jsr mal32		; no, clear
            jsr initclr
            lda #$40
...
ml9         jsr zero2
            dex
            stx dispmk
ml8         jmp clearad2	; activity off, finished
; ------------------------------------------------ Event: Inside
