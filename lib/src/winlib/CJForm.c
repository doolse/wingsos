...
JTab *JFormCreate(HTMLTable *Table, void(*create)(HTMLCell *Cell, void *state), void *state)
{
	...
	do
	{
		...
		switch (cur->Type)
		{
			case WT_STATIC: comp = JStxInit(NULL, cur->Value); break;
			case WT_INNER: comp = (JW *)JFormCreate(cur->Inner, create, state);break;
			case WT_TEXT: comp = JTxfInit(NULL); break;
			case WT_BUTTON: comp = JButInit(NULL, cur->Value); break;
			case WT_TEXTAREA: comp = JTxtInit(NULL); break;
			case WT_FILLER: comp = JFilInit(NULL, 0); break;
		}
		if (comp)
		{
			if (cur->Hints.MinX != -1)
...
static void storeString(uchar *data, uint type, char *value)
{
	uint32 ival;
	switch (type)
	{
